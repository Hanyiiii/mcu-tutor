...
bodyContent.push(body("产品包含五大功能模块：AI 智能答疑、代码生成、Proteus 仿真向导、知识库、学习路径。内置知识库收录 37 条单片机核心知识点（涵盖 8051 基础、Keil 使用、Proteus 仿真、学习方法四大类），13 个可直接编译的代码模板（覆盖流水灯、按键、数码管、定时器、串口、LCD1602、PWM 等经典例程），8 个常用电路的仿真搭建向导，以及 20 项任务的四阶段学习路径。"));
...
bodyContent.push(body("预期成果以可量化指标呈现：内置知识库 37 条、代码模板 13 个、仿真向导 8 个、学习路径任务 20 项，全部功能在线上环境可访问、可演示，并配套项目报告、路演演示文稿与演示视频脚本等参赛材料。"));
...
bodyContent.push(dataTable(
  ...
  [
    ...
    ["知识库", "37 条知识点按分类浏览与关键词搜索；命中条目一键展开", "答疑难"],
...
bodyContent.push(caption("表B-1 内置知识库分类统计（37 条）"));
bodyContent.push(dataTable(
  ["类别", "条数", "内容示例"],
  [
